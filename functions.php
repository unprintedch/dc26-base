...
// Auto-activate required plugins if installed but not active.
...
add_filter('block_type_metadata_settings', function(array $settings, array $metadata): array {
    if (isset($metadata['name']) && $metadata['name'] === 'core/spacer') {
        $settings['attributes']['height']['default'] = 'var(--wp--preset--spacing--50)';
    }
    return $settings;
}, 10, 2);
